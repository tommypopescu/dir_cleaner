...
@app.get("/api/scan")
def api_scan(request: Request, path: Optional[str] = None, size_threshold_mb: Optional[int] = None, unused_days: Optional[int] = None, depth: Optional[int] = None, excludes: Optional[str] = None, auth=Depends(basic_auth)):
    ...
    results = scan_directories(base_path=base, excludes=excl, size_threshold_bytes=size_threshold_bytes, unused_days=u_days, depth=d)
    return {"items": results, "count": len(results)}

@app.post("/api/delete")
async def api_delete(request: Request, base_path: str = Form(None), action: str = Form(...), targets: List[str] = Form(...), auth=Depends(basic_auth)):
    base = Path(base_path) if base_path else BASE_PATH
    quarantine = QUARANTINE_PATH if action == "quarantine" else None
    resp = delete_or_quarantine(base_path=base, targets=targets, quarantine_path=quarantine, audit_log=AUDIT_LOG)
